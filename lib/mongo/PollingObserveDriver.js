...
function listenAll(cursorDescription, listenCallback) {
  ...
  forEachTrigger(cursorDescription, function(trigger) {
    listeners.push(
      DDPServer._InvalidationCrossbar.listen(trigger, listenCallback)
    );
  });
  return {
    stop: function() {
...
export default function PollingObserveDriver(options) {
  ...
  self._ensurePollIsScheduled = _.throttle(
    ...
    self._cursorDescription.options.pollingThrottleMs || 50
    /* ms */
  ); // XXX figure out if we still need a queue

  self._taskQueue = new Meteor._SynchronousQueue();
  var listenersHandle = listenAll(self._cursorDescription, function(
    notification
  ) {
  ...
  if (options._testOnlyPollCallback) {
    ...
    var intervalHandle = Meteor.setInterval(
      _.bind(self._ensurePollIsScheduled, self),
      pollingInterval
    );
...
_.extend(PollingObserveDriver.prototype, {
  ...
  _suspendPolling: function() {
    ...
    if (self._pollsScheduledButNotStarted !== 1)
      throw new Error(
        '_pollsScheduledButNotStarted is ' +
        self._pollsScheduledButNotStarted
      );
  },
      ...
      if (first && typeof e.code === 'number') {
        ...
        self._multiplexer.queryError(
          new Error(
            ...
            JSON.stringify(self._cursorDescription) +
            ': ' +
            e.message
          )
        );
