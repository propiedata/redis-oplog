...
class RedisSubscriptionManager {
  ...
  initializeChannel(channel) {
    ...
    const flushRedisEventsForChannel = debounce(
      () => {
      ...
      { maxWait: Config.maxDebounceWait, trailing: true, leading: false }
    );
  ...
  getDocumentMapForEvents(collection, subscribers, events) {
    ...
    if (docIdsToFetch.length) {
      collection.find({ _id: { $in: docIdsToFetch } }, options).forEach(doc => {
        documentMap[doc._id] = doc;
      });
    }
